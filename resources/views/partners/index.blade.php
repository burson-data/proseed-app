<x-app-layout>
            ...
            <div class="bg-light-surface dark:bg-dark-surface overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-light-text dark:text-dark-text">
                    ...
                    <div class="flex justify-between items-center mb-4">
                        <!-- Kiri: Search & Filter -->
                        <form method="GET" action="{{ route('partners.index') }}" class="flex items-center space-x-4">
                            <input type="text" name="search" placeholder="Search partners..." value="{{ request('search') }}" class="border-gray-300 dark:border-gray-600 dark:bg-dark-surface dark:text-dark-text rounded-md shadow-sm">
                        ...
                        <a href="{{ route('partners.create') }}" class="bg-action hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">
                            Add New Partner
                        </a>
                    </div>
                        ...
                        <table class="min-w-full divide-y divide-gray-200 dark:divide-border-color">
                                ...
                                <tr>
                                    @php
                                        $createSortLink = function ($sortBy, $label) {
                                            ...
                                            $url = route('partners.index', array_merge(request()->query(), ['sort_by' => $sortBy, 'sort_order' => $sortOrder]));
                                            $icon = 'fa-sort';
                                    ...
                                    @endphp
                                    <th class="px-6 py-3 text-left text-xs font-bold text-white uppercase tracking-wider">{!! $createSortLink('partner_name', 'Partner Name') !!}</th>
                                    <th class="px-6 py-3 text-left text-xs font-bold text-white uppercase tracking-wider">{!! $createSortLink('pic_name', 'PIC Name') !!}</th>
                                    <th class="px-6 py-3 text-left text-xs font-bold text-white uppercase tracking-wider">{!! $createSortLink('email', 'Email') !!}</th>
                                    <th class="px-6 py-3 text-left text-xs font-bold text-white uppercase tracking-wider">{!! $createSortLink('is_active', 'Status') !!}</th>
                            ...
                            <tbody class="bg-light-surface dark:bg-dark-surface divide-y divide-gray-200 dark:divide-border-color">
                                @forelse ($partners as $partner)
                                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 cursor-pointer {{ !$partner->is_active ? 'opacity-50' : '' }}" onclick="window.location='{{ route('partners.show', $partner->id) }}';">
                                        <td class="px-6 py-4 whitespace-nowrap">{{ $partner->partner_name }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap">{{ $partner->pic_name }}</td>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-center">
                                            <div onclick="event.stopPropagation();">
                                                <x-dropdown align="right" width="48">
                                                    ...
                                                    <x-slot name="content">
                                                        <x-dropdown-link :href="route('partners.show', $partner->id)">{{ __('View Details') }}</x-dropdown-link>
                                                        <x-dropdown-link :href="route('partners.edit', $partner->id)">{{ __('Edit') }}</x-dropdown-link>
                                                        <x-dropdown-link :href="route('partners.journey', $partner->id)">{{ __('View Journey') }}</x-dropdown-link>
                                                        <div class="border-t border-gray-200 dark:border-gray-600"></div>
                                                        
                                                        <form method="POST" action="{{ route('partners.toggleStatus', $partner->id) }}">
                                                            @csrf
                                                            @method('PATCH')
                                                            <x-dropdown-link :href="route('partners.toggleStatus', $partner->id)" onclick="event.preventDefault(); this.closest('form').submit();" class="{{ $partner->is_active ? 'text-danger' : 'text-success' }}">
                                                                {{ $partner->is_active ? __('Deactivate') : __('Activate') }}
                                                            </x-dropdown-link>
                                                        </form>
                                                    </x-slot>
                                                </x-dropdown>
                                ...
                                @empty
                                    <tr>
                                        <td colspan="5" class="px-6 py-4 text-center text-light-text-muted dark:text-dark-text-muted">
                                            No partners found in this project.
                                        </td>
                                    </tr>
                                @endforelse
                            </tbody>
                    ...
                    <div class="mt-4 flex items-center justify-between">
                        <div>
                            <select id="per_page_select_partners" class="border-gray-300 dark:border-gray-600 dark:bg-dark-surface dark:text-dark-text rounded-md shadow-sm">
    ...
    <script>
        document.getElementById('per_page_select_partners').addEventListener('change', function() {
