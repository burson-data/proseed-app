<x-guest-layout>
    <div class="w-full">
        <h2 class="text-2xl font-bold text-center mb-4 text-white">Select a Project</h2>
                ...
                @foreach($projects as $project)
                    <form method="POST" action="{{ route('projects.select.submit', $project->id) }}">
                        ...
                        <button type="submit" class="w-full text-left p-4 border dark:border-gray-600 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700 focus:outline-none focus:ring">
                            <div class="font-bold text-yellow-400">{{ $project->name }}</div>
        ...
        <div class="border-t dark:border-gray-600 mt-6 pt-4">
            <form method="POST" action="{{ route('logout') }}">
                @csrf
                <button type="submit" class="w-full text-center text-sm text-light-text-muted dark:text-dark-text-muted hover:text-yellow-400 dark:hover:text-dark-text">
                    {{ __('Log Out') }}
                </button>
            </form>
        </div>
